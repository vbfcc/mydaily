...
- `/start` — معرفی + کیبورد
- `/log` یا `📝 ثبت امروز` — شروع ثبت مرحله‌ای (۸ سوال + سوال‌های روتین‌های فعال)
- `/routine` یا `🔁 روتین‌ها` — ساخت/لیست/توقف روتین (با تاریخ شروع و پایان)
- `/profile` یا `👤 حساب کاربری` — نام کاربری، شناسه، تاریخ عضویت شمسی
- `/today` یا `📊 امروز` — نمایش امروز
- `/week` یا `📅 هفته` — ۷ روز گذشته
- `/cancel` — لغو
- `/skip` در مرحله‌ی آخر — رد کردن محرک احساسی

هر شب ساعت ۱۲ (به‌وقت ایران) ربات به همه یادآوری می‌کند گزارش را پر کنند (`bot:remind-midnight` + اسکجولر — روی سرور کرون `schedule:run` لازم است).
