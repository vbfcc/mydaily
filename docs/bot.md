...
- لیست/توقف: `/routine` — لیست متنی + کیبورد منوی اصلی (`mainMenuKeyboard()`)؛ ساخت با `/routine_new` و توقف با `/routine_stop ID`. روتین دارای یادآوری با `⏰ HH:MM` (و `🔕` اگر سایلنت) نمایش داده می‌شود. (هندلرهای `rt:new` / `rt:stop:ID` برای پیام‌های قدیمی نگه داشته شده‌اند.)
